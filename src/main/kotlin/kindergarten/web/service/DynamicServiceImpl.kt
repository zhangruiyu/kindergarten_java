...
interface DynamicService {
    ...
    fun getDynamic(jwt: JwtUser, dynamic_type: Int): Any?
}
...
class DynamicServiceImpl(@Autowired val kgProfileDao: KgProfileDao,
    ...
    override fun getDynamic(jwt: JwtUser, dynamic_type: Int): Any? {
        //用户信息
        val profile = kgProfileDao.selectProfile(jwt.id)
        //判断是用校园id还是教室id获取动态
        val selectId = if (dynamic_type == 0) {
            if (profile.classroomId == null){0}else profile.classroomId
        } else  profile.schoolId
//        dynamicDao.selectDynamic(selectId,dynamic_type)
                return dynamicDao.selectDynamic(selectId,dynamic_type)
    }
}
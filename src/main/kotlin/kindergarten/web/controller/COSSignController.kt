 ...
 */
@RestController
class COSSignController(val ocsConfig: OCSConfig) {
    ...
    @ApiImplicitParams(ApiImplicitParam(name = "type", value = "1是图片动态 2是视频动态", required = true, dataType = "Int"))
    fun getCoSSign(@RequestParam(required = true) type: Int): ResponseData? {
        ...
        val cosPath: String = "$prefix/${CustomConstants.PicType.types[type]}/${jwt.id}"
